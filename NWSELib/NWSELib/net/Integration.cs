...
namespace NWSELib.net
{
    ...
    public class IntegrationRecord
    {
        ...
        static ILog logger = LogManager.GetLogger(typeof(Integration));
        ...
        internal void do_adjust(Network net,Integration inf)
        {
        ...
        public List<Vector> forward_inference(Integration inf, List<Vector> allCondValues)
        {
    ...
    public class Integration : Node
    {
        ...
        /// 推理节点存储的记录
        /// </summary>
        protected List<IntegrationRecord> records = new List<IntegrationRecord>();
        /// <summary>
        /// 推理节点存储的记录
        /// </summary>
        public List<IntegrationRecord> Records { get => this.records; }
        /// <summary>
        /// 新样本
        /// </summary>
        ...
        public Integration(NodeGene gene) : base(gene)
        {
        }
        ...
        #region 信息查询
        public IntegrationGene getGene()
        {
            return (IntegrationGene)gene;
        }
        public List<int> getDimensionList(Network net)
        {
            
            List<int> dimension = new List<int>();
            for (int i = 0; i < ((IntegrationGene)this.gene).dimensions.Count; i++)
            {
                (int id, int t) = ((IntegrationGene)this.gene).dimensions[i];
        ...
        public override Object activate(Network net, int time, Object value = null)
        {
            ...
            List<(int,int)> conds = ((IntegrationGene)this.Gene).getConditions();
            ...
            ((IntegrationGene)this.Gene).sort_dimension();
            ...
            if(max_newsample_density >= min_record_density*2/3)
            {
                ...
                for(int i=0;i<clusters.Count;i++)
                {
                    ...
                    IntegrationRecord newRecord = create_newrecord_bysamples(clusters[i], des[i]);
        ...
        /// <param name="vs"></param>
        /// <returns></returns>
        public IntegrationRecord create_newrecord_bysamples(List<List<Vector>> vs,List<double> densitys=null)
        {
            IntegrationRecord r = new IntegrationRecord();
        ...
        {
            List<(IntegrationRecord, IntegrationRecord)> needMergeRecordPair = new List<(IntegrationRecord, IntegrationRecord)>();
            for(int i=0;i<this.records.Count;i++)
            {
                bool merged = false;
            ...
            {
                IntegrationRecord r1 = needMergeRecordPair[i].Item1;
                IntegrationRecord r2 = needMergeRecordPair[i].Item2;
                ...
                IntegrationRecord newRecord = this.create_newrecord_bysamples(samples);
        ...
        public Vector forwardinference(List<Vector> condvalues)
        {
            ...
            int varindex = ((IntegrationGene)this.gene).getVariableIndex();
        ...
        public (List<Vector>, int,double,int) arginference(string arg)
        {
            ...
            IntegrationRecord argRecord = null;
        ...
        /// <param name="varValue"></param>
        /// <returns></returns>
        public IntegrationRecord getNearestRecord(int index,Vector value)
        {
            if (this.records == null || this.records.Count <= 0) return null;
            double dis = double.MaxValue;
            IntegrationRecord r = null;
            for (int i=0;i<this.records.Count;i++)
            {
        ...
        /// <returns></returns>
        public (List<Vector>, List<Vector>, List<Vector>) splitRecordMeans(Network net, IntegrationRecord record)
        {
            //待返回结果
